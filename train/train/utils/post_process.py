...
def database_aug(gf, top_k=10):
    ...
    time1 = time.time()  # make faiss available
    index = faiss.IndexFlatIP(3072)  # build the index
    index.add(xb)  # add vectors to the index
    # print(index.ntotal)

    # search
    k = 9  # we want to see 4 nearest neighbors
    ...
    dd = gf.copy()
    for i in I:
        gf[i[0]] = 0.9 * dd[i[0]] + 0.1 * (
                    dd[[i[1]]] + dd[i[2]] + dd[i[3]] + dd[i[4]] + dd[i[5]] + dd[i[6]] + dd[i[7]] + dd[i[8]])
    gf = torch.from_numpy(gf).cuda()
    print("dba time", time.time() - time1)
    return gf


def database_aug_after_rerank(gf,distmat, top_k=10):
    # distmat, indices = comput_distmat(gf, gf)
...
#     indices = np.argsort(distmat.cpu().numpy(), axis=1)
#     expanded_gf = 7 * (gf[indices[:, 0]]).mean(dim=1) + (gf[indices[:, :top_k]]).mean(dim=1)
#     expanded_gf = expanded_gf.cuda()
#     return expanded_gf
...
def pca_whiten(feat):
    ...
    pca = pca.fit(feat)
    new_feat = pca.transform(feat)
    print("PCA time",time.time()-time0)
    print(feat.shape,new_feat.shape)
    return torch.tensor(new_feat, device='cuda')
