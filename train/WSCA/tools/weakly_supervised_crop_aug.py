    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            ...
            for j in range(featmap.size(0)):
                ...
                cv2.imshow("img",img)
                cv2.imshow("hot_map",fm)
                cv2.imshow("crop",crop)
                cv2.waitKey(1000)

                crop = cv2.resize(crop, (img_size[1], img_size[0]))
                out_dir = "/home/lab3/bi/0716/Veri/ai_city/tools/output/crop"
                print(out_dir)
                print(os.path.join(out_dir, '/'.join(img_path[j].split('/')[-2:])))
...
def main():
    ...
    parser.add_argument(
        "--config_file", default="/home/lab3/bi/0716/Veri/ai_city/configs/submit.yml", help="path to config file", type=str
    )
    ...
    results = []
    out_dir = '/home/lab3/bi/0716/Veri/ai_cityoutput_crop'
