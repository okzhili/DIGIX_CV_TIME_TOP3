...
def batch_euclidean_distance(qf, gf, N=1000):
    m = qf.shape[0]
    n = gf.shape[0]
...
def batch_torch_topk(qf, gf, k1, N=1000):
    m = qf.shape[0]
    n = gf.shape[0]
...
def re_ranking(probFea, galFea, k1, k2, lambda_value):
    ...
    initial_rank = batch_torch_topk(feat, feat, k1 + 1, N=1000)
    # del feat
    del probFea
    del galFea
    torch.cuda.empty_cache()  # empty GPU memory
    ...
    invIndex = get_inv(all_num,V)
    # a = tqdm(total=all_num)
    # for i in range(all_num):
    #     a.update(1)
    #     invIndex.append(np.where(V[:, i] != 0)[0])
