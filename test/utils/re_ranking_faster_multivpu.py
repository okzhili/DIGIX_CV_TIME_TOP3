...
class myThread (Process):   #继承父类threading.Thread
    def __init__(self, threadID, name, counter):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self.name = name
        self.counter = counter
    def run(self,s,e,V,k):
        for i in range(s,e):
            invIndex_[k].append(np.where(V[:, i] != 0)[0])
...
def euclidean_distance(qf, gf):
    ...
    dist_mat = 1 - qf.mm(gf.t())
    return dist_mat


def batch_euclidean_distance(qf, gf, N=500):
    m = qf.shape[0]
    n = gf.shape[0]
...
def re_ranking(probFea, galFea, k1, k2, lambda_value):
    ...
    initial_rank = batch_torch_topk(feat, feat, k1 + 1, N=500)
    # del feat
    del probFea
    del galFea
    torch.cuda.empty_cache()  # empty GPU memory
    ...
    for i in range(7):
        threads[i].run((i)*num,(i+1)*num,V,i)
    threads[7].run(7*num,all_num,V,7)
    for i in range(8):
        invIndex+=invIndex_[i]
