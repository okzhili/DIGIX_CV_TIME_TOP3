...
def batch_euclidean_distance(qf, gf, N=10000):
    m = qf.shape[0]
    n = gf.shape[0]
...
def batch_torch_topk(qf, gf, k1, N=10000):
    m = qf.shape[0]
    n = gf.shape[0]
...
def re_ranking(probFea, galFea, k1, k2, lambda_value):
    ...
    initial_rank = batch_torch_topk(feat, feat, k1 + 1, N=10000)
    # del feat
    del probFea
    del galFea
    torch.cuda.empty_cache()  # empty GPU memory
    ...
    del V
    gc.collect()  # empty memory
    original_dist = batch_euclidean_distance(feat, feat).numpy()
