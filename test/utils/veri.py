...
class VeRi(BaseImageDataset):
    ...
    def __init__(self, root='./', verbose=True, **kwargs):
        ...
        # query = self._process_dir(self.query_dir, relabel=False)
        # gallery = self._process_dir(self.gallery_dir, relabel=False)
        train = self.get_lable_data('./dianshang/mix_train_balance.pkl',state="train")
        query = self.get_lable_data('./dianshang/huafen_all_crop.pkl',state="query")
        gallery = self.get_lable_data('./dianshang/huafen_all_crop.pkl',state="gallery")
    ...
    def get_lable_data(self,label_path,state):
        ...
        if state=="train":
            ...
            for data in train:
                img_path = os.path.join('dianshang/crop1', data[0][-20:])
                dataset.append((img_path,int(data[1]),0))
        if state=="query":
            train, query, gallery = pickle.load(open(label_path, 'rb'))
            for data in query:
                img_path = os.path.join('dianshang/crop1', data[0])
                dataset.append((img_path,int(data[1]),0))
        if state=="gallery":
            train, query, gallery = pickle.load(open(label_path, 'rb'))
            for data in gallery:
