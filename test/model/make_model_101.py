...
class Backbone_fpn(nn.Module):
    def __init__(self, num_classes, cfg):
        ...
        if model_name == 'EfficientNetb3':
            ...
            print('using cosine layer')
            # self.arcface = CircleLoss(self.in_planes, self.num_classes, s=64.0, m=0.35)
            # #
            # self.classifier_sub1 = CircleLoss(1024, self.num_classes, s=64.0, m=0.35)
            # self.classifier_sub2 = CircleLoss(1024, self.num_classes, s=64.0, m=0.35)
            # self.classifier_sub3 = CircleLoss(1024, self.num_classes, s=64.0, m=0.35)
            # self.arcface = ArcFace(self.in_planes, self.num_classes, s=6.0, m=0.10)
        else:
            self.classifier = nn.Linear(self.in_planes, self.num_classes, bias=False)
            self.classifier.apply(weights_init_classifier)
